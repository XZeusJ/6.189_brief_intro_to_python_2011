...
class Board(object):
    ...
    def __init__(self, win, width, height):
        ...
        for x in range(0,self.width):
        	for y in range(0,self.height):
        		self.block_list[(x,y)] = Block(Point(x, y), 'green')
    ...
    def get_block_neighbors(self, block):
        ...
        # coner_blocklist = [(1,1), (self.width,1), (1,self.height), (self.width, self.height)]
        center_x, center_y = block.x, block.y
        start_x = center_x - 1
        start_y = center_y - 1
        end_x   = center_x + 1
        end_y   = center_y + 1

        if center_x == 0: start_x = center_x
        if center_y == 0: start_y = center_y
        if center_x == self.width -1: end_x = center_x
        if center_y == self.height-1: end_y = center_y
        ...
        for x in range(start_x, end_x+1):
        	for y in range(start_y, end_y+1):
        		if x != center_x or y != center_y:
        			neighbor_blocklist.append(self.block_list[(x,y)])

        return neighbor_blocklist       

    def set_status(self, live_num, block):
    	if live_num < 2: block.new_status = 'dead'
        if live_num > 3: block.new_status = 'dead'
        if live_num == 2 or live_num == 3: block.new_status = 'live'
        if live_num == 3 and (block.is_live() == False): block.new_status = 'live'
    ...
    def simulate(self):
        ...
        for block in self.block_list.values():
        	...
        	for neighbor in neighbors:
        		if neighbor.is_live(): live_num += 1
        	
        	self.set_status(live_num, block)
...
if __name__ == '__main__':    
    ...
    # test_blocklist = [(0,0),(0,1),(1,1),(1,0)]
    # test_blocklist = [(5,5),(5,6),(6,5),(6,6),(0,0),(0,1),(1,1),(1,0)]
    # board.seed(test_blocklist)
